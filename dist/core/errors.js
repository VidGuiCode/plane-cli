...
function getStatusHint(status, path) {
    switch (status) {
        case 401:
        ...
        case 404: {
            ...
            if (path?.includes("modules/"))
                return "Module not found. Check with: plane module list";
            if (path?.includes("labels/"))
                return "Label not found. Check with: plane label list";
            if (path?.includes("states/"))
                return "State not found. Check with: plane state list";
