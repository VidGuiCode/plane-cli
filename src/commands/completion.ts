...
const SUPPORTED_SHELLS: readonly Shell[] = ["bash", "zsh", "fish"];
...
function generateFishCompletion(program: Command): string {
  ...
  const commands = program.commands
    .map(
      (cmd) =>
        `complete -c ${name} -n "__fish_use_subcommand" -a "${cmd.name()}" -d "${cmd.description().replace(/"/g, '\\"')}"`,
    )
    .join("\n");
...
function generateCompletionScript(program: Command, shell: Shell): string {
  switch (shell) {
    case "bash":
