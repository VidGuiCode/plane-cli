...
export function createDiscoverCommand(): Command {
  ...
  command
    ...
    .action(async (opts: { workspace?: string; project?: string }) => {
      try {
        ...
        const reopen =
          inputs.states.find((state) => state.group === "backlog") ??
  ...
  for (const resource of ["states", "members", "labels", "cycles", "modules"] as const) {
    command
      .command(resource)
      ...
      .option(
        "--project <identifier-or-name>",
        "Project identifier or name (overrides active context where required)",
      )
      .action(async (opts: { workspace?: string; project?: string }) => {
        try {
          const config = loadConfig();
